...
if(isset($_GET["TDN"]))
{
    $xoaDuLieu="DELETE FROM khachhang  WHERE TenDangNhap='".$_GET["TDN"]."'";
...
$layDuLieu=phan_trang("*","khachhang","",10,$trang,"");
...
while ($cot = mysqli_fetch_array($truyvan_layDuLieu)) {
    ...
    <tr>
        ...
        <td>
            <?php
            // Kiểm tra $_SESSION['userRole'] để quyết định hiển thị hoặc ẩn nút xóa
            if ($_SESSION['userRole'] == "admin") {
                ?>
                <a href="<?php echo $_SERVER["PHP_SELF"]; ?>?TDN=<?php echo $cot["TenDangNhap"]; ?>"
                   class="XoaDuLieu btn btn-danger">Xóa</a>
                <?php
            }
            ?>
        </td>
